...
class VariantController(http.Controller):
    ...
    @http.route(['/integreat_configurator/create_product_variant'], type='json', auth="user", methods=['POST'])
    def create_product_variant(self, product_template_id, product_template_attribute_value_ids, **kwargs):
        tmpl = request.env['product.template'].browse(int(product_template_id))
        product_id = tmpl.create_product_variant(product_template_attribute_value_ids)
        product = request.env['product.product'].browse(product_id)
        if 'values' in kwargs:
            values = kwargs.get('values')
            values['spec_ancho'] = int(values['spec_ancho'])
            values['spec_largo'] = int(values['spec_largo'])
            product.write(values)
            if not values.get('isLamina', False):
                bom_tmpl = request.env['mrp.bom']._bom_find(product_tmpl=tmpl, product=None,
                                                            picking_type=None, company_id=None, bom_type='normal')
                bom_prod = bom_tmpl.copy()
                bom_prod.write({'product_id': product_id, 'code': product.default_code})
        return product_id
