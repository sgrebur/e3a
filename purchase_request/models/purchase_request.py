...
class PurchaseRequest(models.Model):
    ...
    def action_view_purchase_order(self):
        ...
        lines = self.line_ids.purchase_lines.mapped('order_id').ids
        if len(lines) > 1:
            action.update({
                'view_mode': 'list,form',
                'domain': [('id', 'in', lines)],
            ...
            action.update({
                'view_mode': 'form',
                'res_id': self.line_ids.purchase_lines.mapped('order_id').id,
            })
        return action
    ...
    def action_view_stock_move(self):
        ...
        lines = self.line_ids.purchase_request_allocation_ids.mapped('stock_move_id').ids
        if len(lines) > 1:
            action.update({
                'view_mode': 'list,form',
                'domain': [('id', 'in', lines)],
            ...
            action.update({
                'view_mode': 'form',
                'res_id': self.line_ids.purchase_request_allocation_ids.mapped('stock_move_id').id,
            })
        return action
    ...
    def action_view_purchase_request_line(self):
        self.ensure_one()
        action = {
            'name': _("Líneas de solicitudes de compra"),
            'type': 'ir.actions.act_window',
