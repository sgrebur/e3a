...
class PurchaseRequestLine(models.Model):
    ...
    name = fields.Char(related='request_id.name', store=True)
    ...
    sale_order_id = fields.Many2one(related='sale_line_id.order_id', string='OV')
    production_ids = fields.One2many(related='request_id.group_id.mrp_production_ids', string='OP')
    picking_type_id = fields.Many2one(related='request_id.picking_type_id')

    request_move_ids = fields.One2many(
        comodel_name="stock.move",
        inverse_name="requested_purreq_line_id",
        string="Request from Moves",
    )
    ...
    def button_create_po_wizard(self):
        action = {
            'name': _("Crear OC"),
