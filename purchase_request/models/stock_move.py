...
class StockMove(models.Model):
    ...
    requested_purreq_line_id = fields.Many2one(
        comodel_name="purchase.request.line",
        string="Requested Purchase Request Line",
        ondelete="set null",
        readonly=True,
        copy=False,
    )
    ...
    def _compute_purchase_request_ids(self):
        for rec in self:
            if rec.purchase_request_allocation_ids:
                rec.purchase_request_ids = rec.purchase_request_allocation_ids.purchase_request_id
            else:
