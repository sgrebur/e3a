...
class StockRule(models.Model):
    ...
    def _prepare_purchase_request_line(self, request_id, procurement):
        ...
        return {
            ...
            "date_required": "date_planned" in procurement.values and procurement.values["date_planned"]
                             or fields.Datetime.now(),
            "product_uom_id": procurement.product_id.uom_po_id.id,
            "product_qty": procurement_uom_po_qty,
            "request_id": request_id.id,
            "move_dest_ids": [(4, x.id) for x in procurement.values.get("move_dest_ids", [])],
            "request_move_ids": [(4, x.id) for x in procurement.values.get("request_move_ids", [])],
            "orderpoint_id": procurement.values.get("orderpoint_id", False)
                             and procurement.values.get("orderpoint_id").id,
            "sale_line_id": procurement.values.get("sale_line_id", False) and procurement.values.get("sale_line_id")
        }
