...
class PurchaseRequestLineMakePurchaseOrder(models.TransientModel):
    ...
    different_picking_types = fields.Boolean(compute='_compute_picking_type')

    def _compute_picking_type(self):
        self.different_picking_types = False
        for line in self.item_ids:
            line_picking_type = line.request_id.picking_type_id or False
            if line_picking_type != self.picking_type_id:
                self.different_picking_types = True
    ...
    def default_get(self, fields):
        ...
        res["picking_type_id"] = request_lines[0].request_id.picking_type_id.id
    ...
    def make_purchase_order(self):
        ...
        for item in self.item_ids:
            ...
            if not purchase:
                ...
                new_group_id = self.env['procurement.group'].create({
                    'name': purchase.name,
                    'partner_id': purchase.partner_id.id
                })
                purchase.write({'group_id': new_group_id})
            ...
            if not any(po is False for po in line.request_id.line_ids.purchase_request_allocation_ids.purchase_line_id):
        ...
        for po_line in purchase.order_line:
            seller = po_line.product_id.seller_ids.filtered(lambda r: r.name == partner and r.product_id == po_line.product_id).sorted(key=lambda r: r.min_qty)
            if seller:
                moq_qty = seller[0].min_qty
                if moq_qty > po_line.product_qty:
                    # The onchange quantity is altering the scheduled date of the PO
                    # lines. We do not want that:
                    date_required = po_line.date_planned
                    po_line.product_qty = moq_qty
                    po_line._onchange_quantity()
                    po_line.date_planned = date_required
